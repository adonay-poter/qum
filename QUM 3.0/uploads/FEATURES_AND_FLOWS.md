...
| **Letter to future you** | Private `user_letters` row; optional onboarding + home edit; random Phase 2 / victory surfacing |
...
| **Task system** | DB + offline cache of `physical` / `cognitive` / `mindful` tasks; difficulty tier scales with resilience |
...
| **Account** | Header account menu → **Settings** (letter, voice memo, reflections, calm-hour, find support, sign out) |
...
3. **Auth gate**: no user → `AuthScreen` or `VerifyEmailScreen` if signup pending verification.
...
| Sign up | `AuthScreen` | Create account → verification required |
| Verify | `VerifyEmailScreen` | Blocks until email confirmed |
...
| 5 | **Letter to future you** (optional): textarea, 200–500 chars suggested; **Save and continue** or **Skip for now** |
...
**Telemetry:** `onboarding_addiction_selected`, `voice_memo_recorded`, `voice_memo_skipped`, `letter_skipped_in_onboarding`
...
| **Settings** | Account menu → letter, voice memo, reflections, calm-hour, find support, sign out |
...
| Item | Destination |
|------|-------------|
| Edit letter | `LetterEditorScreen` |
...
| **Cold** | Trust-based 90s countdown after “I'm doing it”; face/neck/wrists cold water | 90s elapsed **or** phase timer ≥ 180s |
| **Stillness** | 5→4→3→2→1 sensory grounding; tap per item or step timer (30/24/18/12/15s) | All steps done **or** phase timer ≥ 180s |

Telemetry: `phase1_modality_selected` with `{ modality }`.
...
| **tasks** | Seeded physical / cognitive / mindful prompts |
